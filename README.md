...
**الرصيد المستحق وأعمار الديون والالتزام** جميعها تُحسب من نفس مصدر تقرير **Partner Ledger** في أودو (`account.move.line` على حساب العميل المدين `property_account_receivable_id` / `account_type = asset_receivable`) بدل الفواتير فقط — بالضبط نفس المصدر المستخدم في صفحة "كشف الحساب"، حتى تتطابق كل الأرقام مع ما تراه في `/odoo/partner-ledger`:

- **الرصيد المستحق** = مجموع `amount_residual` لكل قيود حساب العميل المدين غير المسواة بالكامل (نفس منطق أودو للتسوية الجزئية).
- **أعمار الديون** تُحسب من `date_maturity` (تاريخ استحقاق كل بند) مقابل تاريخ اليوم، على الجزء غير المسدد (`amount_residual`) من كل بند — وليس من تاريخ استحقاق الفاتورة ككل، فتلتقط الفروقات مثل السداد الجزئي والقيود اليدوية.
- **الالتزام** = من بنود الفواتير (مدين) التي تجاوزت تاريخ استحقاقها: نسبة التي أُقفلت بالكامل (`amount_residual ≈ 0`) إلى إجمالي المستحقة.
...
- **المبيعات** لا تزال تُحسب من الفواتير (`account.move`, `out_invoice`/`out_refund`) لأنها الأنسب لرسم الاتجاه الشهري.
- **التحصيل** يُحسب من سجلات `account.payment` (دفعات واردة من نوع عميل، بحالة مرحّلة) لنفس سبب رسم الاتجاه الشهري.

هذه القواعد قابلة للتعديل بسهولة داخل `src/lib/analytics.ts` و `src/lib/receivables.ts` حسب سياسة الشركة الائتمانية.
