...
export const api = {
  ...
  // Opening a window only after an await is what browsers' popup blockers
  // silently swallow, since it no longer counts as a direct user gesture.
  contractCaseTemplateUrl: (caseId) => {
    const session = getSession();
  ...
  performanceReport: (period, asOf) => request(`/api/reports/performance?period=${period}${asOf ? `&as_of=${asOf}` : ""}`),
  exportPerformanceReport: (period, asOf) => requestBlob(`/api/reports/performance/export?period=${period}${asOf ? `&as_of=${asOf}` : ""}`),
  ...
  getDiscountCase: (partnerId, params = {}) => {
    ...
    return request(`/api/admin/discount-case/${partnerId}${suffix}`);
  },
  ...
  setUserFullCustomerAccess: (id, full_customer_access) =>
    request(`/api/admin/users/${id}/full-customer-access?full_customer_access=${full_customer_access}`, { method: "PATCH" }),
  ...
  dailyActivityReport: (asOf) => request(`/api/admin/daily-activity${asOf ? `?as_of=${asOf}` : ""}`),
